...
namespace Ted.Limit.ExtLoader
{
    partial class ShareUnit : IShareUnit, IExtLoader
    {
        ...
        {
            if (string.IsNullOrEmpty(suText) || suText.Trim().Equals(""))
            {
                return "There is no any modules.";
            }
            string loadMsg = "";
            try
            {
        ...
        Ted.Limit.Core.IMake IShareUnit.Add(string ExtDefPath, out string key)
        {
            ...
            try
            {
                ...
                if (make == null)
                {
                    throw new ExtLoadingException("未能成功加载扩展模块!");
                }
            ...
            {
                throw new ExtLoadingException("加载扩展模块过程中发生不可控异常!", exp);
            }
            key = ext.Key;
            return make;
        }
            ...
            get 
            {
                ...
                for (int i = 0; i < m_suLineLst.Count; i++)
                {
                    ...
                    if (!suLine.IsBlankLine)
                    {
                        if (suLine.HasCode)
                        {
                            text += suLine.Code + CRLF;
                        }
                        if (suLine.HasComment)
                        {
                            text += suLine.Comment + CRLF;
                        }
                    }
                }
                return text;
        ...
        /// <returns></returns>
        string IShareUnit.GetResourcePath(string moduleKey)
        {
            ShareUnitLine suLine = m_ExtPool[moduleKey];
            IExt ext = suLine.ReflectExt;
            return ((DefaultExt)ext).Resource;
        }
        ...
        /// </summary>
        /// <returns></returns>
        Ted.Limit.Core.IMake[] IShareUnit.AllModules()
        {
            List<Ted.Limit.Core.IMake> makeLst = new List<Ted.Limit.Core.IMake>();
            foreach (ShareUnitLine suLine in m_suLineLst)
            {
                if (suLine.ReflectExt != null)
                {
                    makeLst.Add(suLine.ReflectExt.Make);
                }
            }
            return makeLst.ToArray();
        }
